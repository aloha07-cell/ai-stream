...
# @FileName:    live_man.py
# @Time:        2024/1/2 21:51
# @Author:      bubu
# @Project:     douyinLiveWebFetcher
# @Modified:    Integrated with comment queue and comment saving
...
from modules.comment_capture.ac_signature import get__ac_signature
from modules.comment_capture.protobuf.douyin import *
...
class CommentSaver:
    ...
    def __init__(self, save_file='comments.json'):
    ...
    def save_comment(self, comment_type, data):
        """保存弹幕"""
        comment_data = {
            'type': comment_type,
...
def init_comment_saver(save_file='comments.json'):
    """初始化弹幕保存器"""
    global comment_saver
    comment_saver = CommentSaver(save_file)
...
def generateSignature(wss, script_file='sign.js'):
    ...
    script_file = 'modules/comment_capture/sign.js'
...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id, abogus_file='modules/comment_capture/a_bogus.js', comment_queue=None, save_comments=True, save_file='comments.json'):
        """
        直播间弹幕抓取对象
        :param live_id: 直播间的直播id，打开直播间web首页的链接如：https://live.douyin.com/261378947940，
                        其中的261378947940即是live_id
        :param comment_queue: 用于存放评论的队列
        :param save_comments: 是否保存弹幕到文件
        :param save_file: 弹幕保存文件路径
        """
    ...
    def _parseChatMsg(self, payload):
        ...
        if self.save_comments and comment_saver:
            comment_saver.save_comment('chat', {
                'user_id': user_id,
                'user_name': user_name,
                'content': content
            })
        
        # Add comment to queue if available
        if self.comment_queue:
            try:
                from utils.helpers import sanitize_comment, is_valid_comment
                from config.config import MAX_COMMENT_QUEUE_SIZE
                
                sanitized_comment = sanitize_comment(content)
                if is_valid_comment(sanitized_comment):
                    comment_data = {
                    ...
                    if self.comment_queue.qsize() < MAX_COMMENT_QUEUE_SIZE:
                        ...
                        print(f"Added comment to queue: {user_name}: {sanitized_comment}")
            except Exception as e:
    ...
    def _parseGiftMsg(self, payload):
        ...
        # Save gift to file
        if self.save_comments and comment_saver:
            comment_saver.save_comment('gift', {
                'user_name': user_name,
                'gift_name': gift_name,
                'gift_count': gift_cnt
            })
    ...
    def _parseLikeMsg(self, payload):
        ...
        # Save like to file
        if self.save_comments and comment_saver:
            comment_saver.save_comment('like', {
                'user_name': user_name,
                'like_count': count
            })
    ...
    def _parseMemberMsg(self, payload):
        ...
        # Save member to file
        if self.save_comments and comment_saver:
            comment_saver.save_comment('member', {
                'user_id': user_id,
                'user_name': user_name,
                'gender': gender
            })
    ...
    def _parseSocialMsg(self, payload):
        ...
        # Save follow to file
        if self.save_comments and comment_saver:
            comment_saver.save_comment('follow', {
                'user_id': user_id,
                'user_name': user_name
            })
    ...
    def _parseRoomUserSeqMsg(self, payload):
        ...
        # Save stats to file
        if self.save_comments and comment_saver:
            comment_saver.save_comment('stats', {
    ...
    def _parseFansclubMsg(self, payload):
        ...
        # Save fansclub to file
        if self.save_comments and comment_saver:
            comment_saver.save_comment('fansclub', {
                'content': content
            })
