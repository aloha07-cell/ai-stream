...
本次更新添加了真实的抖音直播弹幕抓取功能：

1. **使用 `douyin-live` 库**：用于抓取真实的抖音直播弹幕
2. **智能切换**：如果 `douyin-live` 库不可用，会自动切换到模拟数据
3. **配置简单**：只需在 `.env` 文件中填写抖音直播间URL或房间ID

### 关键改动：
- 添加了 `douyin-live` 库到依赖
- 更新了 `comment_capture.py` 以使用真实的弹幕抓取
- 更新了部署文档，添加了使用说明
